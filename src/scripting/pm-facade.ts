...
export function installPmFacade(transport: FacadeTransport): void {
  ...
  function createCryptoJsShim(): any {
    const K = [
    ...
    const stringToBytes = (str: any): any => {
      const bytes: number[] = []
    ...
    return { SHA256(message: any) { const bytes = typeof message === 'string' ? stringToBytes(message) : Array.isArray(message) ? message : stringToBytes(String(message ?? '')); const hashHex = sha256(bytes); return { toString: () => hashHex } } }
  }
