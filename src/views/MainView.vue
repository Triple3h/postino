  ...
  <div class="main-layout">
    ...
    <div class="main-content">
      <div class="toolbar">
        ...
        <div class="toolbar-actions">
          ...
          <label class="cors-mode">
            <span>网络模式</span>
            <select v-model="store.settings.corsMode" class="cors-select" @change="saveNetworkSettings">
          ...
          <button class="btn btn-sm" @click="showRightPanel = !showRightPanel" :class="{ active: showRightPanel }" title="Ctrl+Shift+R">
            {{ showRightPanel ? '隐藏右栏' : '右侧面板' }}
          </button>
          <button class="btn btn-sm" @click="openShortcutSettings" title="自定义快捷键">
            快捷键
          </button>
      ...
      <div class="content-area">
        ...
        <div v-if="showRightPanel" class="right-panel-resizer" title="拖拽调整右侧面板宽度" @mousedown="startRightPanelResize"></div>
        <aside v-if="showRightPanel" class="right-inspector" :style="{ width: rightPanelWidth + 'px' }">
          ...
          <section class="inspector-card">
            ...
            <div v-if="currentEnv?.variables.length" class="env-var-mini-list">
              <button v-for="v in currentEnv.variables.filter(item => item.enabled)" :key="v.key" class="env-var-mini" @click="copyText('{{' + v.key + '}}')">
          ...
          <section class="inspector-card">
            <h3>📋 动态值</h3>
            <button v-for="item in dynamicValues" :key="item" class="dynamic-token" @click="copyText(item)">{{ item }}</button>
          </section>
          <section class="inspector-card">
            <h3>🔖 接口信息</h3>
